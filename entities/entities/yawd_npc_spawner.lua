...
local rndSnd = {Sound("ambient/creatures/rats1.wav"),Sound("ambient/creatures/rats2.wav"),Sound("ambient/creatures/rats3.wav")}
function ENT:Think()
    local d = LocalPlayer():GetPos():Distance(self:GetPos())
    if not (self._nsnd and self._nsnd > CurTime()) and d < 100 then
        self._nsnd = CurTime() + 5
        local snd = table.Random(rndSnd)
        self:EmitSound(snd,50, math.random(20,40))
    end
